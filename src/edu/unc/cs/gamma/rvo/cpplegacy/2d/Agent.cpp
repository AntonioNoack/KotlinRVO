		...
		for (size_t i = 0; i < obstacleNeighbors_.size(); ++i) {
			...
			for (size_t j = 0; j < orcaLines_.size(); ++j) {
				...
				orcaLines_[j].direction) - invTimeHorizonObst * radius_ >= -RVO_EPSILON &&
				 det(invTimeHorizonObst * relativePosition2 - orcaLines_[j].point, orcaLines_[j].direction)
				  - invTimeHorizonObst * radius_ >=  -RVO_EPSILON) {
					alreadyCovered = true;
					break;
				}
			}
			...
			if (s < 0.0f && distSq1 <= radiusSq) {
				...
				if (obstacle1->isConvex_) {
					...
					line.direction = normalize(Vector2(-relativePosition1.y(),
					 relativePosition1.x()));
			...
			if (det(lines[i].direction, lines[i].point - result) > distance) {
				...
				for (size_t j = numObstLines; j < i; ++j) {
					...
					if (std::fabs(determinant) <= RVO_EPSILON) {
						...
						line.point = lines[i].point + (det(lines[j].direction, lines[i].point - lines[j].point) /
				...
				if (linearProgram2(projLines, radius, Vector2(-lines[i].direction.y(), lines[i].direction.x()),
